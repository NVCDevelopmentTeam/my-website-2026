...
>
  ...
  {#if isLoading}
    <div class="fixed top-0 left-0 right-0 h-1 bg-sky-600 dark:bg-sky-400 z-[9999]">
      <div
        class="h-full w-full bg-white/40 shadow-[0_0_10px_rgba(255,255,255,0.5)] animate-progress origin-left"
      ></div>
    </div>
  {/if}
  ...
  <main id="main-content" class="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
    <div class="lg:grid lg:grid-cols-12 lg:gap-12 py-12">
      <div
        class="lg:col-span-8 transition-opacity duration-150 {isLoading
          ? 'opacity-90'
          : 'opacity-100'}"
      >
        {@render children?.()}
